...
- `--ink` `--ink-strong` `--ink-soft` `--ink-faint` `--gold` `--teal` は、`--paper` `--card` `--paper2` のどの背景に置いても **コントラスト比 4.5 以上**（明るい配色・暗い配色の両方で検査）
- バッジの `--tone-*-fg` / `--tone-*-bg`、`--tag-fg` / `--tag-bg` も同様
- `font-size` は **11px 以上**
...
端末の設定に従って自動で切り替わります。`--navy` は「濃紺の面」の色なので、**見出しの文字色には使わないでください**（暗い配色で地色と同化します）。文字色は `--ink-strong` を使います。明るい側・暗い側の両方を `validate.mjs` が検査します。
